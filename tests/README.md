...
│   ├── tasks-page.ts            # поддельная страница задач для tasks_fix.js без логина
...
`exams-dashboard` устроен так же: настоящее расширение, подставная страница
списка курсов и ответы API через `context.route`. Сервер расписания контрольных
ему не нужен — расписание кладётся прямо в кэш расширения
(`futureExamsScheduleCache` со свежей отметкой времени), и 30 минут расширение в
сеть за ним не ходит. Даты в расписании считаются от сегодняшнего дня: номер
недели расширение считает по своим часам, а подменить их контент-скрипту
...
курсы» — 66rem, и у группы курсов такой же `min-width`. Поэтому места сбоку
проверяются в настоящих размерах: окно 1536 × 737 (экран 1920 × 1080 при
масштабе 125 %), колонка справа и слева помещается целиком, а после
разворачивания меню дэшборд уходит полоской над курсами и возвращается, когда
меню свернули.
