...
async function waitForTasksLoaded(page) {
  ...
  await page.waitForSelector('tr[class*="task-table__task"]', {
    state: 'attached',
    timeout: 15_000,
  });
...
async function getVisibleTaskRows(page) {
  return page
    .locator('tr[class*="task-table__task"]')
    .filter({ has: page.locator(':not([style*="display: none"])') })
    .evaluateAll((rows) => rows.filter((r) => r.style.display !== 'none').length);
}
...
async function getCurrentCourseNames(page): Promise<string[]> {
  return page
    .locator('tr[class*="task-table__task"] .task-table__course-name')
    .evaluateAll((els) => [...new Set(els.map((el) => el.textContent.trim()).filter(Boolean))]);
}
...
test.describe('Issue #216: фильтр курсов при смене семестра', () => {
  ...
  test('явно снятый пользователем курс остаётся снятым после перезагрузки', async ({ page }) => {
    ...
    const totalForCourse = await page
      .locator('tr[class*="task-table__task"]')
      .evaluateAll(
        (rows, course) =>
          rows.filter(
