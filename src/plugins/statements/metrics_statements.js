...
(async function () {
  ...
  async function recalculateAndUpdateUI() {
    ...
    const archivedCourseIds = archivedCourseHrefs.map((href) =>
      parseInt(href.split('/').pop(), 10)
    );

    const archivedCourses = allCoursesData.filter((course) =>
      archivedCourseIds.includes(course.id)
    );
    const nonArchivedCourses = allCoursesData.filter(
      (course) => !archivedCourseIds.includes(course.id)
    );
