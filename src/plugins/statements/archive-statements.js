...
(async function () {
    ...
    if (archivedCourses.size === 0) {
      ...
      archivedCourses.forEach((course) => {
        ...
        link.addEventListener('click', (e) => {
          ...
          if (!mainFieldset) {
            window.cuLmsLog(
              '[LMS Extension] ❌ Main fieldset not found! Falling back to hard nav.'
            );
          ...
          if (originalLink) {
            ...
            window.cuLmsLog(
              `[LMS Extension] ❌ Original link not found. Falling back to hard nav.`
            );
  ...
  function cleanup() {
    ...
    document
      .querySelectorAll('.lms-archive-btn, .lms-unarchive-btn')
  ...
  browser.storage.onChanged.addListener((changes, namespace) => {
    if (namespace === 'sync' && changes.themeEnabled) {
      ...
      window.cuLmsLog(
        `[LMS Extension] Theme changed to ${isDarkTheme ? 'dark' : 'light'}, icons updated`
      );
    }
  });
})();
