...
(async function () {
  ...
  const COURSE_INFO_URL_TEMPLATE =
    'https://my.centraluniversity.ru/api/micro-lms/courses/{courseId}/exercises';
  ...
  function findBestCourseMatch(apiCourseName, courseData) {
    ...
    for (const key in courseData) {
      if (key.toLowerCase().trim() === apiNameLower) return courseData[key];
    }
    ...
    const processedApiName = apiNameLower
      .replace(/(?:\s+|\.|_)(?:поток|группа|group|stream)?\s*\d+$/, '')
      .trim();
    if (processedApiName !== apiNameLower) {
      for (const key in courseData) {
        if (key.toLowerCase().trim() === processedApiName) return courseData[key];
      }
    }
    return null;
  }
    ...
    if (mode === 'endOfCourse' && courseConfig) {
      for (const configName in courseConfig) {
        ...
        if (bestMatchName && maxScore > 0.7) {
          ...
          finalActivities.set(configNameLower, {
            name: displayName,
            weight: courseConfig[configName].weight,
            scores: [],
            sum: 0,
          });
        }
  ...
  async function fetchCourseName(courseId) {
    ...
    try {
      const response = await fetch(COURSE_INFO_URL_TEMPLATE.replace('{courseId}', courseId));
  ...
  function renderSidebar(data, parentComponent, tableWrapper, calculationMode) {
    ...
    if (!document.getElementById(STYLE_ID)) {
      ...
      styleSheet.innerHTML =
        `${PARENT_COMPONENT_SELECTOR} { position: relative !important; padding-right: 580px !important; box-sizing: border-box !important; } .adv-table-count-col { text-align: center; width: 80px; } #${CONTAINER_ID} .cu-table th.adv-table-count-col, #${CONTAINER_ID} .cu-table td.adv-table-count-col { border-right: 1px solid rgba(255, 255, 255, 0.12); }`
          .replace(/\s\s+/g, ' ')
          .trim();
    ...
    const explanation =
      calculationMode === 'endOfCourse'
        ? 'Средний балл вычисляется по <b>всем</b> заданиям, запланированным в курсе (согласно настройкам плагина). Задания без оценки учитываются как 0.'
        : 'Средний балл вычисляется только по тем заданиям, где <b>уже выставлена оценка</b> (включая 0). Задания без оценки не учитываются.';
