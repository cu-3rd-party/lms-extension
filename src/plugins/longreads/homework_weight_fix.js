...
function findWeightInApiResponse(data) {
  ...
  const itemWithWeight = data.items.find(
    (item) => item?.estimation?.activity?.weight !== undefined
  );
...
function insertWeightElement(infoList, weight) {
  ...
  if (anchorItem) {
    ...
    if (scoreIcon) {
      ...
      const valueSpan =
        weightListItem.querySelector('span.font-text-m') ||
        weightListItem.querySelector('.task-info__item-title + span');
...
function persistentIconReplacer() {
  ...
  const iconObserver = new MutationObserver(() => {
    const iconsToReplace = document.querySelectorAll(
      `img[src*="${originalIconSrcPart}"]:not([${processedAttribute}])`
    );
            ...
            if (r.ok) {
              ...
              if (!errorLabel) {
                ...
                errorLabel.style.cssText =
                  'color: red; font-family: "Inter", sans-serif; display: block; margin-top: 8px;';
                errorLabel.textContent =
                  'Нет доступных Late Days для отмены – до дедлайна менее 24 часов';
