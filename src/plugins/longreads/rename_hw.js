...
(function () {
  ...
  function findWeekNumberInUI() {
    ...
    for (const element of weekElements.reverse()) {
      const weekMatch =
        element.textContent.match(/неделя\s*(\d+)/i) || element.textContent.match(/(\d+)/);
  ...
  function handleStorageChanges(changes, area) {
    ...
    if (changed) {
      log(
        `Обновлены настройки авто-переименования: enabled=${renameEnabled}, template='${currentTemplate}'`
      );
    }
  }
