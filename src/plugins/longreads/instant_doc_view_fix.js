      ...
      for (const item of taskItems) {
        ...
        if (taskDetails) {
          const allAtts = [
            ...
            ...(taskDetails.content?.attachments || []),
          ];
          const foundInTask = allAtts.find((att) => att.name === fullDisplayedFileName);
          if (foundInTask) {
            foundFilename = foundInTask.filename;
            foundVersion = foundInTask.version;
            break;
          }
        }
        ...
        if (comments && Array.isArray(comments)) {
          for (const comment of comments) {
            if (comment.attachments?.length) {
              const foundInComment = comment.attachments.find(
                (att) => att.name === fullDisplayedFileName
              );
              if (foundInComment) {
                foundFilename = foundInComment.filename;
                foundVersion = foundInComment.version;
                break;
              }
            }
          }
