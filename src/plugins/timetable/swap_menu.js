...
if (typeof window.__culmsSwapMenuInit === 'undefined') {
  ...
  function buildMenu() {
    ...
    section.innerHTML = `
      ...
      <p class="culms-swap-menu__lead">
        ...
        контакты друг друга. Дальше меняетесь сами, а если оба слота забиты
        наглухо — через куратора.
      </p>
  ...
  function renderMatch(match) {
    ...
    hint.textContent =
      'Свяжитесь и договоритесь. Сначала попробуйте пересесть сами: если хотя бы ' +
      'в одном из слотов есть свободное место, один переходит туда, а второй ' +
      'занимает освободившееся. Если мест нет нигде, так не получится — тогда ' +
      'напишите куратору, он поменяет вас вручную.';
