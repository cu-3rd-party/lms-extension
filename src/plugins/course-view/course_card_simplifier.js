  ...
  courseCards.forEach((li) => {
    ...
    if (maxLength && courseName.length > maxLength) {
      ...
      if (lastSpaceIndex > 0) {
        ...
        const thrownOutWord =
          nextSpaceIndex > 0 ? remainingText.substring(0, nextSpaceIndex) : remainingText;
...
function observeCourseListChanges() {
  ...
  courseListObserver = new MutationObserver(() => {
    if (
      document.querySelector('ul.course-list') &&
      !document.querySelector('.simplified-course-card')
    ) {
