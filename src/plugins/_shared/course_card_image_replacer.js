...
(function () {
  ...
  function getCourseId(iconNode) {
    ...
    const card =
      iconNode.closest('.t-card') || iconNode.closest('cu-course-card') || iconNode.parentNode;
    if (card) {
    ...
    for (const mutation of mutations) {
      for (const node of mutation.addedNodes) {
        if (node.nodeType === Node.ELEMENT_NODE) {
          ...
          const iconsInside = node.querySelectorAll
            ? node.querySelectorAll(TARGET_ICON_SELECTOR)
            : [];
