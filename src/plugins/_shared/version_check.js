...
(function () {
  ...
  async function getLatestVersionFromGist() {
    try {
      ...
      if (!response.ok) {
        console.error(
          '[VersionCheck] Не удалось получить данные с GitHub. Статус:',
          response.status
        );
        return null;
      }
      ...
      if (data.files && data.files['version.json']) {
        ...
        if (content[specificKey]) {
          console.log(
            `[VersionCheck] Используется версия для ${browserName}: ${content[specificKey]}`
          );
          return content[specificKey];
        } else {
          console.log(
            `[VersionCheck] Версия для ${browserName} не найдена, используется общая: ${content.version}`
          );
  ...
  function showUpdateNotification() {
    ...
    const originalLateDaysItem = headerActionsList.querySelector(
      'li[automation-id="header-action"]'
    );
  ...
  async function checkVersion() {
    const storedData = await api.storage.local.get([
      'lastVersionCheckTimestamp',
      'cachedLatestVersion',
    ]);
    ...
    if (latestVersion) {
      if (isUpdateAvailable(latestVersion, CURRENT_PLUGIN_VERSION)) {
        console.log(
          `[VersionCheck] Доступна новая версия: ${latestVersion}. Текущая: ${CURRENT_PLUGIN_VERSION}`
        );
        showUpdateNotification();
      } else {
        console.log(
          `[VersionCheck] Обновление не требуется. (Gist: ${latestVersion}, Local: ${CURRENT_PLUGIN_VERSION})`
        );
        removeUpdateNotification();
      }
    }
