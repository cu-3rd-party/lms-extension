...
var API_COURSES_URL =
  API_COURSES_URL || '/api/micro-lms/courses/student?limit=10000&state=published';
...
function bindFriendsLogic() {
  ...
  const saveStoredFriends = (friends) =>
    localStorage.setItem('cu_friends_list', JSON.stringify(friends));
  ...
  const renderList = async () => {
    ...
    if (friends.length === 0) {
      ...
      list.innerHTML =
        '<div style="color: var(--fr-text-sec); font-size: 16px; text-align: center; padding: 40px;">Пока никого нет.<br>Введите имя в поиске выше.</div>';
      return;
    } else {
    ...
    friends.forEach((friend, index) => {
      ...
      busyBtn.addEventListener('click', async (e) => {
        ...
        schedBox.style.display = 'flex';
        schedBox.innerHTML =
          '<span style="color: var(--fr-text-sec);">Анализ (текущая неделя)...</span>';
          ...
          if (subjects !== null) {
            ...
            if (subjects.length > 0) {
              ...
              subjContainer.innerHTML =
                '<span style="color: var(--fr-text-sec); font-size: 12px; font-style: italic;">Нет предметов</span>';
            }
          } else {
